function lambda =  update_lambda(X,Xr,Xs,Xv,z,beta,sigma,eta,lambda,etas,lambdas,betav,qv,delta,psijh,pi,nf,ncr,spatial,factorcov,speciesX,includeXs,includeXv)
[ny ns] = size(z);
if length(spatial)==0
	np = [];
	nr = 0;
...
for r1=1:nr
	...
	if factorcov(r1) == 1
		...
		for k=1:ncr(r1)
			...
			Plam(:,:,k) = bsxfun(@times,psijh1(:,:,k),tau(:,k)');
		end
		eta1 = XrEta;
		eta1=eta1(lpi,:);
		eta2 = eta1'*eta1;
