...
for k=0:(length(q)+dispTrue)
   ...
   switch saveToFile
      case 'csv'
         [status, message, messgaeid] = mkdir(fullfile(m.folder,'results'));
         if ~status
            error(aprintf('HMSC: cannot create results folder:\n%s', message));
         end
         if k == 0
      ...
      case 'xls'
         ...
         fileName = fullfile(m.folder,'results',strcat(name,'.xlsx'));
         A = table2cell(tab);
         if k == 0
            [status, message] = xlswrite(fileName,A, 1);
         elseif k <= length(q)
            [status, message] = xlswrite( fileName,A, sprintf('quantile %s', num2str(q(k))) );
         else
