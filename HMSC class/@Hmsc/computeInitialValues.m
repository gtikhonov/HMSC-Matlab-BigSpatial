function [X, covScale, Xs, sCovScale, Xv, vCovScale, Xr, factorCovScale, beta,gamma,iV,sigma,rho,ph,z,nf,lambda,eta,delta,psijh,alpha,nfs,lambdas,etas,deltas,psijhs,betav,qv] = computeInitialValues(m, start)
...
if m.includeXv
	[vCovScale, Xv] = Hmsc.scaleMatrix(m.Xv, m.vCovScaleFlag, m.includeXv-1);
else
	vCovScale = [];
	Xv = [];
end
...
if ~isempty(start)
	...
	rho = p.rho;
	ph = p.ph;
	if islogical(m.fixNf)
		nf = p.nf;
	else
		error('HMSC: Fix number of factors parameter must be boolean if specifying MCMC starting parameter set');
	end
	lambda = p.lambda;
	for r=1:m.nr
	...
	betav = p.betav;
   qv = p.qv;
	z = Hmsc.update_z(m.Y, X,m.Xr,m.Xs,beta,eta,lambda,etas,lambdas,m.Y,m.pi,m.ncr,m.dist,sigma,m.speciesX,m.includeXs,m.factorCov,1);
	%z = m.Y;
	...
	if m.nr > 0
        ...
        elseif isnumeric(m.fixNf) && length(m.fixNf) == m.nr
			nf = m.fixNf;
        else
            error('HMSC: Fix number of factors parameter must be boolean or numeric vector of length equal to number of random factors levels');
		end
	else
		nf = [];
	end
	if islogical(m.fixNfs) | isempty(m.fixNfs)
		if m.ncs > 0
			nfs = 2;
		else
			nfs = 0;
		end
	elseif isnumeric(m.fixNfs)
      nfs = m.fixNfs;
	end
	sigma = eye(m.ns);
	...
	z = Hmsc.update_z(m.Y,X,m.Xr,m.Xs,m.Xv,beta,eta,lambda,etas,lambdas,betav,qv,m.Y,nf,m.pi,m.ncr,m.dist,sigma,m.speciesX,m.includeXs,m.includeXv,m.factorCov,1);
	z(isnan(z))=0;
end
