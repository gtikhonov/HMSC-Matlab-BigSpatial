function etas= update_etas(X,Xs,Xv,Xr,nf,pi,ncr,z,lambdas,betav,qv,beta,sigma,eta,lambda,spatial,speciesX,includeXs,includeXv,factorcov)
[nfs ns]=size(lambdas);
if length(spatial)==0
	np = [];
	nr = 0;
...
for r1=1:nr
	...
	if factorcov(r1)
		Xr1 = Xr{r1};
		for k1=1:ncr(r1)
			XrEta = repmat(Xr1(:,k1), 1, nf(r1)).*eta1;
...
if includeXs == 2
	...
	Uetas = invChol_mex( eye(ncs*nfs)+tmp*Ksi );
%    Uetas = inv( eye(ncs*nfs)+tmp*Ksi );
	Uetas = (Uetas+Uetas')/2;
	metas = Uetas*tmp*S(:);
else
	[ny, ncs]=size(Xs);
	Uetas=invChol_mex( eye(ncs*nfs)+kron(lambdas*isigma*lambdas',Xs'*Xs) );
