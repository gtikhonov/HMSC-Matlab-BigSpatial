function Y = predictConditional(m, n, Yc, nmcmc, X, Xs,Xv, piCell, xyCell, XrCell, expected)
...
for rN = 1:n
		...
		if m.spatial(i) && newPiN > 0
			...
			for j = 1:p.nf(i)
				...
				if alpha > 0
					...
					etaN(:, j) = randn([newPiN,1]);
				end
			end
		else
			etaN = randn([newPiN,p.nf(i)]);
		end
		eta{i} = [etaM; etaN];
	end
	...
	for i=1:nmcmc 
		z=m.update_z(z,X,Xr,Xs,Xv,p.beta,eta,p.lambda,p.etas,p.lambdas,p.betav,p.qv,Yc,p.nf,pi,m.ncr,m.dist,p.sigma,m.speciesX,m.includeXs,m.includeXv,m.factorCov);
		if (i<nmcmc)
            eta=m.update_eta(z,X,Xr,Xs,Xv,p.beta,p.sigma,eta,nalpha,p.lambda,p.etas,p.lambdas,p.betav,p.qv,p.nf,pi,m.ncr,m.spatial,m.factorCov,iWgA,m.speciesX,m.includeXs,m.includeXv);
		end
	end
