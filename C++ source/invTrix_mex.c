...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]){
	...
	mwSize n, K;   /* Size of the matrix and the 3d dimension */
	...
	if (nrhs>2 || nrhs==0) {
		mexErrMsgIdAndTxt("mexFunction:invTrix_mex:numInputs",
		 "not more than 2 input required: A (square matrix), [UpLo='U' ({'U','L'})]");
	} else if (nrhs==2){
		if(mxIsChar(prhs[1]) != 1){
			mexErrMsgIdAndTxt("MATLAB:invTrix_mex:illegaltype", "Second arguments must be either 'U' or 'L' ");
		}
    ...
    if (n!=dimVec[1]) {
        mexErrMsgIdAndTxt("MATLAB:invTrix_mex:matchdims",
                "matrix is not square");
    }

	/* check for real matrix */
	if (mxIsComplex(prhs[0])) {
		mexErrMsgIdAndTxt("MATLAB:invTrix_mex:iscomplex",
		 "matrix must be real");
	}
	...
	if(type==mxDOUBLE_CLASS){
		for(k=0; k<K; ++k){
			...
			dtrtri( uplo, diag, &n, B, &n, &info ); /* Double triangular inversion */
			
			// mexErrMsgIdAndTxt("MATLAB:invTrix_mex:dpotri:singularmatrix",
					  // "B %lf", B[3]);
			
			/* check for success */
			if (info<0) {
				mexErrMsgIdAndTxt("MATLAB:invTrix_mex:dpotri:illegalvalue",
		...
		for(k=0; k<K; ++k){
			...
			strtri( uplo, diag, &n, B, &n, &info ); /* Float triangular inversion */

			/* check for success */
			if (info<0) {
				mexErrMsgIdAndTxt("MATLAB:invTrix_mex:spotri:illegalvalue",
					"failed to invert: illegal value");
			}

			if (info>0) {
				mexErrMsgIdAndTxt("MATLAB:invTrix_mex:spotri:singularmatrix",
					  "failed to invert: a diagonal element was 0");
			}
		}
		 
	} else {
		mexErrMsgIdAndTxt("MATLAB:invTrix_mex:illegaltype",
